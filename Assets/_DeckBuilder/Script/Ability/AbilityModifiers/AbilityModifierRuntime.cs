...
{
    public static Dictionary<AbilityStatKey, float> Evaluate(IReadOnlyList<AbilityTagSO> tags, IEnumerable<AbilityStatEntry> baseStats, IEnumerable<AbilityModifier> activeModifiers)
    {
        ...
        var abilityTags = tags ?? new List<AbilityTagSO>();

        var matching = activeModifiers.Where(mod => mod && mod.Querry.Matches(abilityTags)).ToList();
