...
{
    private static readonly string[] OldAssemblies =
    {
        "Assembly-CSharp",
        "DeckBuilder"
    };

    private static readonly Dictionary<string, string> TargetAssemblies = new()
    {
        { nameof(GameObjectList), "DeckBuilder.Entity" },
        { nameof(UseAbility), "DeckBuilder.Entity" }
    };

    private const string DefaultAbilityAssembly = "DeckBuilder.Ability";
    ...
    public static void RemapAbilityBehaviours()
    {
        ...
        Debug.Log($"Remap Ability Behaviours: updated {changedAssets} asset(s), {totalReplacements} managed reference(s) remapped to feature assemblies.");
    }
    ...
    private static Dictionary<string, string> BuildReplacements()
    {
        ...
        {
            string targetAssembly = TargetAssemblies.TryGetValue(typeName, out string mappedAsm)
                ? mappedAsm
                : DefaultAbilityAssembly;

            foreach (string oldAsm in OldAssemblies)
            {
                string oldFragment = $"class: {typeName}, ns: , asm: {oldAsm}";
                string newFragment = $"class: {typeName}, ns: , asm: {targetAssembly}";
